...
const BASELINE_BACKEND_DIR = process.env.BASELINE_BACKEND_DIR || '/tmp/janggi-backend-baseline/backend';
const PROJECT = process.env.REFACTOR_BENCH_PROJECT || 'janggi-backend-refactor';
const PORTS = {
  baseline: 3301,
  refactor: 3302,
};
const COMPOSE_PATH = path.join(os.tmpdir(), 'janggi-backend-refactor-compose.yml');
const REPORTS_DIR = path.join(ROOT, 'reports');
const RAW_RESULTS_PATH = path.join(REPORTS_DIR, 'backend-refactor-results.json');
const REPORT_PATH = path.join(REPORTS_DIR, 'backend-refactor-report.md');
...
function ensureBaseline() {
  if (!fs.existsSync(BASELINE_BACKEND_DIR)) {
    throw new Error(`Baseline backend snapshot not found at ${BASELINE_BACKEND_DIR}`);
  }
}
...
function writeComposeFile() {
  ...
  postgres-refactor:
    ...
    environment:
      ...
      POSTGRES_DB: janggi_db
    volumes:
      - ${path.join(ROOT, 'backend', 'init.sql')}:/docker-entrypoint-initdb.d/init.sql:ro
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U janggi_user -d janggi_db"]
      interval: 5s
      timeout: 5s
  ...
  mock-ai:
    build: ${path.join(ROOT, 'tools', 'backend-refactor', 'mock-ai')}
...
function parseCoverage(dir) {
  const output = sh('go', ['test', '-cover', './...'], { cwd: dir });
  const match = output.match(/coverage:\s+([0-9.]+)% of statements/);
...
function buildReport(results) {
  ...
  const lines = [];
  lines.push('# Backend Refactor Report', '');
  ...
  lines.push('| Metric | Baseline backend | Refactored backend |');
  lines.push('| --- | ---: | ---: |');
...
async function main() {
  ...
  try {
    ...
    const refactorMetrics = {
      loc: countLoc(path.join(ROOT, 'backend'), ['*.go', 'init.sql']),
      files: collectFiles(path.join(ROOT, 'backend'), ['*.go', 'init.sql']).length,
      deps: parseGoDependencies(path.join(ROOT, 'backend')),
      coverage: parseCoverage(path.join(ROOT, 'backend')),
    };
