# API 서버 문서 (Go + Gin + Socket.IO 호환 실시간 서버 + PostgreSQL)

## 1. 범위
API 서버는 서비스의 권한/상태 기준(Authoritative) 계층으로 다음을 담당합니다.
- JWT 인증/세션 수명주기
- 유저 정보/회원 탈퇴
...
진입점: `api-server/cmd/api-server/main.go`
...
- `POST /api/ai/move` (api-server -> ai-server 중계)
...
## 12. 계층형 구조
- `cmd/api-server/main.go` – 실행 진입점과 의존성 조립
- `internal/domain/` – 순수 보드/랭크/AI 레벨/수 파싱/유틸 로직
- `internal/application/` – 유스케이스 오케스트레이션, 세션 처리, 실시간 대국 수명주기
- `internal/dataaccess/` – PostgreSQL helper와 migration 실행
- `internal/platform/` – 설정 로딩, AI HTTP client, frontend dist 서빙
- `internal/presentation/http/` – Gin router 조립
- `internal/presentation/socket/` – Socket.IO server 조립
