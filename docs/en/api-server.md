# API Server Documentation (Go + Gin + Socket.IO-Compatible Realtime + PostgreSQL)

## 1. Scope
API server is the core authoritative service for:
...
Entry point: `api-server/cmd/api-server/main.go`
...
- `POST /api/ai/move` (api-server -> ai-server relay)
...
- `cmd/api-server/main.go` – executable entrypoint and dependency wiring
- `internal/domain/` – pure board, rank, AI level, move parsing, and utility logic
- `internal/application/` – use-case orchestration, session handling, and realtime lifecycle
- `internal/dataaccess/` – PostgreSQL helpers and migration execution
- `internal/platform/` – config loading, AI HTTP client, and frontend dist serving
- `internal/presentation/http/` – Gin router composition
- `internal/presentation/socket/` – Socket.IO server composition
