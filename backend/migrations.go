...
func (app *app) initDB(ctx context.Context) error {
	queries := []string{
		...
		fmt.Sprintf(`ALTER TABLE users ADD COLUMN IF NOT EXISTS ai_unlocked_tier INTEGER DEFAULT %d`, defaultAITier),
		fmt.Sprintf(`UPDATE users SET ai_unlocked_tier = LEAST(%d, GREATEST(%d, COALESCE(ai_unlocked_tier, %d)))`, maxAITier, defaultAITier, defaultAITier),
		`UPDATE users SET rank_wins = COALESCE(rank_wins, 0), rank_losses = COALESCE(rank_losses, 0)`,
		fmt.Sprintf(`ALTER TABLE users ADD COLUMN IF NOT EXISTS rating INTEGER DEFAULT %d`, eloDefaultRating),
		fmt.Sprintf(`UPDATE users SET rating = %d WHERE rating IS NULL OR rating = 0`, eloDefaultRating),
